...
import { IGetOrderDetails, IGetOrderHistory, IOrderItem } from '../../../types/backend'
...
const AdminOrderDetails: React.FC = () => {
    ...
    const [order, setOrder] = useState<IGetOrderDetails>()
    ...
    useEffect(() => {
        const getOrderItems = async () => {
            if (orderId) {
                ...
                const orderResponse: any = await api.getOrderDetails(orderId);

                if (orderResponse.data) {
                    setOrder(orderResponse.data)
                    console.log(orderResponse.data.status)
                }
    ...
    return (
        <div className="surface-section px-4 py-8 md:px-6 lg:px-8">
            ...
            <div className="flex flex-column sm:flex-row sm:align-items-center sm:justify-content-between pt-5">
                <div className="mb-3 sm:mb-0">
                    <span className="font-medium text-xl text-900 mr-2">User name:</span>
                    <span className="font-medium text-xl text-blue-500">{order?.userName}</span>
                </div>
            </div>
